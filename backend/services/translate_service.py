...
def translate(text: str, target_lang: str) -> TranslateResponse:
    ...
    if genai_client.mode == "live":
        ...
        lookup_key = normalized_text + "?" if "restroom" in normalized_text or "exit" in normalized_text or "food" in normalized_text or "seat" in normalized_text else normalized_text
        ...
        if matched_dict and normalized_lang in matched_dict:
            return TranslateResponse(
                translated_text=matched_dict[normalized_lang],
                mode="mock"
            )
