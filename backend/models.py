...
AccessibilityNeed = Literal["none", "wheelchair", "visual", "hearing", "cognitive", "elderly"]
...
class UserContext(BaseModel):
    ...
    """
    persona: Persona = Field(default="fan", description="The user persona navigating the stadium.")
    stadium_id: str = Field(..., min_length=1, max_length=50, description="The unique identifier of the stadium.")
    gate: Optional[str] = Field(None, max_length=20, description="Optional entry gate.")
    seat_section: Optional[str] = Field(None, max_length=20, description="Optional seating section.")
    language: str = Field("en", min_length=2, max_length=8, description="Language preference (e.g. 'en', 'es').")
    accessibility_need: AccessibilityNeed = Field("none", description="Accessibility accommodations requested.")
    minutes_to_kickoff: Optional[int] = Field(None, ge=-180, le=360, description="Remaining minutes until match kickoff.")
...
class ChatRequest(BaseModel):
    ...
    """
    message: str = Field(..., min_length=1, max_length=1000, description="The user's query or message.")
    context: UserContext = Field(..., description="The context of the user sending the message.")
...
class ChatResponse(BaseModel):
    ...
    suggested_actions: List[str] = Field(default_factory=list, description="Context-aware quick actions.")
...
class TranslateRequest(BaseModel):
    ...
    target_lang: str = Field(..., min_length=2, max_length=8, description="Target language code (e.g. 'es').")
...
class ZoneStatus(BaseModel):
    ...
    density_level: DensityLevel = Field(..., description="Categorized crowd density level.")
    density_pct: int = Field(..., ge=0, le=100, description="Simulated density percentage.")
    recommendation: str = Field(..., description="Actionable recommendation for this zone.")
...
class CrowdResponse(BaseModel):
    ...
    generated_at: str = Field(..., description="ISO 8601 timestamp of simulation generation.")
    zones: List[ZoneStatus] = Field(..., description="List of stadium zones with statuses.")
    overall_recommendation: str = Field(..., description="Overall narrative recommendation.")
...
class TransportOption(BaseModel):
    ...
    eta_minutes: int = Field(..., ge=0, description="Estimated arrival/waiting time in minutes.")
    accessibility_friendly: bool = Field(..., description="Whether this option supports accessibility needs.")
...
class TransportResponse(BaseModel):
    ...
    """
    options: List[TransportOption] = Field(..., description="List of available transport choices.")
    recommendation: str = Field(..., description="GenAI recommendation for the best option.")
...
class SustainabilityResponse(BaseModel):
    ...
    """
    tips: List[str] = Field(..., description="List of personalized sustainability tips.")
    nearest_recycling_zone: str = Field(..., description="Name of the closest recycling point.")
    nearest_water_refill: str = Field(..., description="Name of the closest water refill station.")
